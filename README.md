...
This project started as a creative 404 error page for [texascabinetryco.com](https://texascabinetryco.com) and has since grown into a complete, self-contained arcade shooter. Built with vanilla HTML/CSS/JavaScript — no build step, no dependencies needed. The compiled game engine is embedded directly in the self-contained `404.shtml` deployment file.
...
- **Perk System:** Pick an upgrade after every wave including Rapid Fire, Multi-Shot, Piercing Rounds, Thruster Boost, Salvage Magnet, Reinforced Hull, Combo Mastery, Score Surge, plus rare perks like Auto-Fire, Vector Thrusters, Orbital Cannon, Chain Lightning, and Homing Rounds
- **Powerups:** Rapid Fire (stacks with repeats), Shield (absorbs 3 hits), 1UP (extra life), Salvage Magnet (pulls pickups toward you)
- **Overheat / Risk:** Rapid Fire overheat risk after six stacks, with heat damage if pushed too far
- **Leaderboard:** Local top-3 leaderboard with initials entry, stored per-browser via `localStorage`
- **Attract Mode:** Self-playing AI runs when idle, showcasing the leaderboard and top score; includes best-run input replay stored locally per browser
...
You can also open `404.shtml` directly in a browser for the self-contained deployment file.

## Deploy

Drop `index.html` or `404.shtml` in as your site's 404 page:
- Rename `index.html` to `404.shtml` or use the provided `404.shtml`
